use crate::{
    FoldError, disk_backed_queue::DiskBackedQueue, interner::Interner, memory_config::MemoryConfig,
...
impl CheckpointManager {
    ...
    pub fn with_base_dir(base_dir: &Path) -> Self {
        Self {
            ...
            temp_checkpoint_dir: base_dir
                .join("checkpoint_temp")
                .to_string_lossy()
                .to_string(),
    ...
    pub fn save(
        ...
        fs::write(
            format!("{}/interner.bin", self.temp_checkpoint_dir),
            interner_bytes,
        )
        ...
        fs::rename(&self.temp_checkpoint_dir, &self.checkpoint_dir)
            .map_err(|e| FoldError::Io(e))?;
    ...
    pub fn load(
    ...
    ) -> Result<Option<(Interner, DiskBackedQueue, SeenTracker)>, FoldError> {
        ...
        let mut temp_queue =
            DiskBackedQueue::new_from_path(&self.results_temp, memory_config.queue_buffer_size)?;
        ...
        let mut new_results =
            DiskBackedQueue::new_from_path(&self.results_path, memory_config.queue_buffer_size)?;
...
mod tests {
    ...
    fn test_save_and_load_checkpoint() {
        ...
        assert_eq!(
            loaded_tracker.len(),
            2,
            "Tracker should have 2 IDs reconstructed from results"
        );
    ...
    fn test_checkpoint_rehydrates_bloom_from_all_results() {
        ...
        // Add three distinct orthos to results (bloom/seen will be reconstructed from these)
        let ortho1 = Ortho::new();
        let ortho2 = ortho1.add(0).into_iter().next().unwrap();
        let ortho3 = ortho2.add(1).into_iter().next().unwrap();
        let id1 = ortho1.id();
        let id2 = ortho2.id();
        let id3 = ortho3.id();
        ...
        assert_eq!(
            loaded_tracker.len(),
            3,
            "Tracker should have 3 IDs reconstructed from all results"
        );
        assert!(loaded_tracker.contains(&id1));
        assert!(loaded_tracker.contains(&id2));
