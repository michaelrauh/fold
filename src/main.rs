...
fn main() -> Result<(), FoldError> {
    ...
    for (file_idx, file_path) in input_files.iter().enumerate() {
        ...
        let (new_interner, seeded_count) = fold::process_text(
            ...
            &mut optimal_ortho, 
            &mut ortho_storage,
            move |queue_len, total_found, bloom_hits, bloom_misses, disk_checks, queue_mem, queue_disk| {
                if quit_check.load(std::sync::atomic::Ordering::Relaxed) {
                    return;
                }
                let mut state_lock = state_clone.lock().unwrap();
                state_lock.update_metrics(queue_len, total_found);
                state_lock.update_cache_stats(bloom_hits, bloom_misses, disk_checks, queue_mem, queue_disk);
            }
        )?;
