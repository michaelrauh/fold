...
fn test_txt_recovery_removes_all_partial_work() {
    ...
    assert!(
        recovered_txt.exists(),
        "Txt file should be recovered to input/"
    );
...
fn test_merge_recovery_restores_archives_and_removes_partial_work() {
    ...
    let results_merged_path = config
        .base_dir
        .join(format!("results_merged_{}", std::process::id()));
    let mut results =
        DiskBackedQueue::new_from_path(results_merged_path.to_str().unwrap(), 1000).unwrap();
    results.push(Ortho::new()).unwrap();
    drop(results);

    // Get paths
    let merge_work_folder = format!(
        "{}/merge_{}.work",
    ...
    assert!(
        std::path::Path::new(&merge_work_folder).exists(),
    ...
    assert!(
        recovered_a.exists(),
        "Archive A should be recovered to input/"
    );
    assert!(
        recovered_b.exists(),
        "Archive B should be recovered to input/"
    );
    assert!(
