...
fn test_simple_worker_loop() {
    ...
    // Act
    let (interner, _changed_keys_count, frontier_size, _impacted_frontier_count, _processed) = fold::process_text(text, None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_multiple_file_processing() {
    ...
    // Act
    for text in texts {
        let (new_interner, _changed_keys_count, _frontier_size, _impacted_frontier_count, _processed) = fold::process_text(text, interner, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_optimal_ortho_tracking() {
    ...
    // Act
    let (_interner, _changed_keys_count, _frontier_size, _impacted_frontier_count, _processed) = fold::process_text(text, None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_interner_version_increments() {
    ...
    // Act
    let (interner1, _, _, _, _processed) = fold::process_text("first text", None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    let (interner2, _, _, _, _processed) = fold::process_text("second text", Some(interner1), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    let (interner3, _, _, _, _processed) = fold::process_text("third text", Some(interner2), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_changed_keys_tracking() {
    ...
    let (interner1, changed_count1, _, _, _processed) = fold::process_text("a b c", None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    ...
    let (interner2, changed_count2, _, _, _processed) = fold::process_text("a c", Some(interner1), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    ...
    let (_interner3, changed_count3, _, _, _processed) = fold::process_text("x y z", Some(interner2), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_frontier_tracks_leaf_orthos() {
    ...
    // Act
    let (_interner, _changed_keys_count, frontier_size, _impacted_frontier_count, _processed) = fold::process_text(text, None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_frontier_accumulates_across_files() {
    ...
    for text in texts {
        let (new_interner, _changed_keys_count, frontier_size, _impacted_frontier_count, _processed) = fold::process_text(text, interner, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_impacted_frontier_count_with_no_changes() {
    ...
    let (interner1, changed_count1, _, impacted_count1, _processed) = fold::process_text("a b c", None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    ...
    let (_interner2, changed_count2, _, impacted_count2, _processed) = fold::process_text("", Some(interner1), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_impacted_frontier_count_with_changes() {
    ...
    // Act - first text (baseline)
    let (interner1, changed_count1, frontier_size1, impacted_count1, _processed) = fold::process_text("a b", None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    ...
    let (_interner2, changed_count2, frontier_size2, impacted_count2, _processed) = fold::process_text("a c", Some(interner1), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_impacted_frontier_count_with_new_vocabulary() {
    ...
    // Act - first text
    let (interner1, _, _, _, _processed) = fold::process_text("hello world", None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    
    // Act - second text with completely new vocabulary
    let (_interner2, changed_count2, frontier_size2, impacted_count2, _processed) = fold::process_text("foo bar", Some(interner1), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
...
fn test_impacted_frontier_count_accumulates_correctly() {
    ...
    let (interner1, _, _, impacted1, _processed) = fold::process_text("a b c", None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    assert_eq!(impacted1, 0, "First should have 0 impacted");
    
    let (interner2, changed2, _, impacted2, _processed) = fold::process_text("a d", Some(interner1), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    if changed2 > 0 {
    ...
    let (_interner3, changed3, frontier_size3, impacted3, _processed) = fold::process_text("e f", Some(interner2), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    if changed3 > 0 {
...
fn test_impacted_frontier_orthos_contain_changed_keys() {
    ...
    let (interner1, _, _, _, _processed) = fold::process_text("the cat", None, &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
    
    // Act - Add text that changes existing key completions
    let (_interner2, changed_count, frontier_size, impacted_count, _processed) = fold::process_text("the dog", Some(interner1), &mut seen_ids, &mut optimal_ortho, &mut frontier, &mut HashMap::new()).expect("process_text should succeed");
