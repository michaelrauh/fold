...
fn test_diagonal_calculation_2x2() {
    ...
    for pos in 0..4 {
        let (prefixes, diagonals) = spatial::get_requirements(pos, &dims, None);
        println!(
            "Position {}: prefixes={:?}, diagonals={:?}",
            pos, prefixes, diagonals
        );
    }
    ...
    assert_eq!(
        diagonals,
        vec![1],
        "Position 2 should have position 1 in its diagonal"
    );
...
fn test_diagonal_calculation_3x3() {
    ...
    let (_prefixes, diagonals) = spatial::get_requirements(4, &dims, None);
    assert_eq!(
        diagonals,
        vec![3],
        "Position 4 should have position 3 in its diagonal (both at distance 2)"
    );

    // Position 5 [2,0]: distance 2 - positions 3 [0,2] and 4 [1,1] are at distance 2 and come before
    let (_prefixes, diagonals) = spatial::get_requirements(5, &dims, None);
    assert_eq!(
        diagonals,
        vec![3, 4],
        "Position 5 should have positions 3 and 4 in its diagonal"
    );
}
