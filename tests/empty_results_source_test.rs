...
fn test_empty_text_produces_archive_with_no_results() -> Result<(), FoldError> {
    ...
    println!(
        "Processing complete. Processed {} orthos, results count: {}",
        processed_count,
        results.len()
    );
    ...
    assert_eq!(
        loaded_results.len(),
        0,
        "Bug: Archive loaded with 0 results"
    );
...
fn test_single_word_produces_minimal_results() -> Result<(), FoldError> {
    ...
    println!(
        "Single-word interner vocab size: {}",
    ...
    println!(
        "Single-word: Processed {} orthos, results count: {}",
        processed_count,
        results.len()
    );
