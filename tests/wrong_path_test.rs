...
fn test_save_with_wrong_results_path() -> Result<(), FoldError> {
    ...
    println!(
        "Created DiskBackedQueue at: {}",
        actual_results_path.display()
    );
    ...
    assert!(
        files_at_actual.len() > 0,
        "Should have files at actual path"
    );
    ...
    println!(
        "save_archive will look for: {}",
        wrong_results_path.display()
    );
...
fn test_stored_path_vs_method_call() -> Result<(), FoldError> {
    ...
    let ingestion =
        file_handler::ingest_txt_file_with_config(input_file.to_str().unwrap(), &config)?;
