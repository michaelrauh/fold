...
fn test_incomplete_merge_recovery_causes_empty_archives() -> Result<(), FoldError> {
    ...
    let mut partial_results =
        DiskBackedQueue::new_from_path(results_merged_path.to_str().unwrap(), 10)?;
    ...
    let merge_work_folder = in_process_entries
        .iter()
    ...
    if merge_work_path.exists() {
        println!(
            "✗ BUG: Merge work folder NOT cleaned up: {}",
            merge_work_folder
        );
    } else {
    ...
    if results_merged_path.exists() && !new_results_path.exists() {
        println!("\n✗ BUG CONFIRMED:");
        println!(
            "  - Old results_merged_{} has files but wrong PID",
            fake_pid
        );
        println!(
            "  - New merge uses results_merged_{} (empty/non-existent)",
            new_pid
        );
...
fn test_recovery_preserves_active_merge() -> Result<(), FoldError> {
    ...
    let merge_work = config
        .in_process_dir()
        .join(format!("merge_{}.work", fake_pid));
