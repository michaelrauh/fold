...
fn test_exact_txt_processing_save_flow() -> Result<(), FoldError> {
    ...
    let ingestion =
        file_handler::ingest_txt_file_with_config(input_file.to_str().unwrap(), &config).map_err(
            |e| {
    ...
    if Path::new(&results_path).exists() {
        let files: Vec<_> = fs::read_dir(&results_path)?.collect();
        println!(
            "Results directory has {} files after manual flush",
            files.len()
        );
    }

    // Now call save_result - this is what happens in main.rs
    let (archive_path, _lineage) =
        ingestion.save_result(&interner, results, Some(&Ortho::new()), 1)?;
