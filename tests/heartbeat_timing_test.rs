...
fn test_archives_in_input_have_no_heartbeat() {
    ...
    // Save the archive
    let (archive_path, _) = ingestion
        .save_result(&interner, results, Some(&ortho), 1)
        .unwrap();

    // Verify archive is in input and has NO heartbeat
    let archive_heartbeat = format!("{}/heartbeat", archive_path);
    assert!(
        !std::path::Path::new(&archive_heartbeat).exists(),
...
fn test_archives_in_process_get_heartbeat_when_moved() {
    ...
    let ingestion_a =
        file_handler::ingest_txt_file_with_config(txt_file_a.to_str().unwrap(), &config).unwrap();
    ...
    let (archive_path_a, _) = ingestion_a
        .save_result(&interner_a, results_a, Some(&ortho_a), 1)
        .unwrap();
    ...
    let ingestion_b =
        file_handler::ingest_txt_file_with_config(txt_file_b.to_str().unwrap(), &config).unwrap();
    ...
    let (archive_path_b, _) = ingestion_b
        .save_result(&interner_b, results_b, Some(&ortho_b), 1)
        .unwrap();
    ...
    assert!(
        !std::path::Path::new(&archive_a_heartbeat_before).exists(),
        "Archive A in input/ should not have a heartbeat"
    );
    assert!(
        !std::path::Path::new(&archive_b_heartbeat_before).exists(),
        "Archive B in input/ should not have a heartbeat"
    );

    // Now ingest them for merging (this moves them to in_process)
    let merge_ingestion =
        file_handler::ingest_archives_with_config(&archive_path_a, &archive_path_b, &config)
            .unwrap();
