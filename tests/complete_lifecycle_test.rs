...
fn test_complete_txt_lifecycle() {
    ...
    assert!(std::path::Path::new(&work_folder).exists(), "Work folder should exist");
    assert!(std::path::Path::new(&source_txt).exists(), "source.txt should exist");
    assert!(std::path::Path::new(&heartbeat).exists(), "Heartbeat should be created");
    ...
    assert!(std::path::Path::new(&results_path).exists(), "Results should exist");
    ...
    assert!(!std::path::Path::new(&work_folder).exists(), "Work folder should be deleted");
    assert!(!std::path::Path::new(&results_path).exists(), "Results should be deleted");
    
    // === STEP 8: Can be worked again ===
    let ingestion2 = file_handler::ingest_txt_file_with_config(
        recovered_txt.to_str().unwrap(),
        &config
    ).unwrap();
    ...
    assert!(std::path::Path::new(&work_folder2).exists(), "Work folder should exist again");
    assert!(std::path::Path::new(&heartbeat2).exists(), "New heartbeat should exist");
...
fn test_complete_merge_lifecycle() {
    ...
    assert!(!archive_a.join("heartbeat").exists(), "Archive A should NOT have heartbeat in input");
    assert!(!archive_b.join("heartbeat").exists(), "Archive B should NOT have heartbeat in input");
    ...
    let merge_work = format!("{}/merge_{}.work", config.in_process_dir().display(), std::process::id());
    ...
    assert!(std::path::Path::new(&merge_work).exists(), "Merge work folder should exist");
    assert!(std::path::Path::new(&merge_heartbeat).exists(), "Merge heartbeat should exist");
    
    // === STEP 3: Merge work begins - queues and results created ===
    let work_queue_path = ingestion.work_queue_path();
    let seen_shards_path = ingestion.seen_shards_path();
    let results_merged = config.base_dir.join(format!("results_merged_{}", std::process::id()));
    ...
    let mut results = DiskBackedQueue::new_from_path(results_merged.to_str().unwrap(), 1000).unwrap();
    results.push(Ortho::new()).unwrap();
    
    println!("✓ Step 3: Merge work directories created");
    assert!(std::path::Path::new(&work_queue_path).exists(), "Merge queue should exist");
    assert!(std::path::Path::new(&seen_shards_path).exists(), "Merge shards should exist");
    ...
    assert!(new_mtime > original_mtime, "Merge heartbeat should be updated");
    ...
    assert!(!std::path::Path::new(&merge_work).exists(), "Merge work should be deleted");
    ...
    let merge_work2 = format!("{}/merge_{}.work", config.in_process_dir().display(), std::process::id());
    
    println!("✓ Step 8: Archives can be merged again");
    assert!(archive_a_in_process2.exists(), "Archive A should be in in_process again");
    assert!(std::path::Path::new(&merge_work2).exists(), "New merge work should exist");
