...
fn test_end_to_end_single_file_workflow() {
    ...
    assert!(stage_output.status.success(), 
        "stage.sh failed: {}", String::from_utf8_lossy(&stage_output.stderr));
    
    println!("[test] stage.sh output: {}", String::from_utf8_lossy(&stage_output.stdout));
    ...
    println!("[test] Chunk {}: '{}'", chunk.file_name().to_string_lossy(), content.trim());
    assert_eq!(content.trim(), test_content, "Chunk content should match input");
    ...
    assert!(fold_output.status.success(), 
        "Fold processing failed: {}", String::from_utf8_lossy(&fold_output.stderr));
    ...
    let archives: Vec<_> = remaining_files.iter()
        .filter(|e| {
            e.path().is_dir() && 
    ...
    println!("[test] Checking archive: {}", archive.file_name().to_string_lossy());
    ...
    assert!(optimal_content.contains("yellow"), "Expected 'yellow' in geometry");
    assert!(optimal_content.contains("orange"), "Expected 'orange' in geometry");
    assert!(optimal_content.contains("purple"), "Expected 'purple' in geometry");
    assert!(optimal_content.contains("black"), "Expected 'black' in geometry");
    assert!(optimal_content.contains("white"), "Expected 'white' in geometry");
    assert!(optimal_content.contains("gray"), "Expected 'gray' in geometry");
    ...
    assert_eq!(lineage_content, "\"test_input_chunk_0001\"", "Expected exact lineage match");
    ...
    println!("[test] Results directory has {} file(s)", results_files.len());
    
    // Verify no txt files remain (all consumed)
    let txt_files: Vec<_> = remaining_files.iter()
        .filter(|e| {
            e.path().is_file() && 
