...
fn test_orphaned_archive_has_heartbeat_removed() {
    ...
    assert!(heartbeat_path.exists(), "Heartbeat should exist in in_process before recovery");
    ...
    assert!(recovered_archive.exists(), "Orphaned archive should be recovered to input/");
    
    // Heartbeat should NOT exist in recovered archive
    let recovered_heartbeat = recovered_archive.join("heartbeat");
    assert!(!recovered_heartbeat.exists(), 
