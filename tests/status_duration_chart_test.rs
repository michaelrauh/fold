...
fn test_status_history_tracking() {
    ...
    assert!(snapshot.status_history.len() >= 2, 
    ...
    assert!(snapshot.status_history[0].duration >= 2, 
    ...
    assert!(snapshot.status_history[1].duration >= 2, 
        "Duration should be at least 2 seconds, got {}", 
...
fn test_status_history_capacity() {
    ...
    assert_eq!(snapshot.status_history.len(), 9,
    ...
    for (idx, entry) in snapshot.status_history.iter().enumerate() {
        assert_eq!(entry.status, format!("Status_{}", idx),
            "Entry {} should be Status_{}", idx, idx);
