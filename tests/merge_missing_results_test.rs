...
fn test_merge_scenario_with_actual_archives() -> Result<(), FoldError> {
    ...
    println!("Archive A results directory has {} files", results_a_files.len());
    assert!(results_a_files.len() > 0, "Archive A should have result files");
    ...
    println!("Archive B results directory has {} files", results_b_files.len());
    assert!(results_b_files.len() > 0, "Archive B should have result files");
