...
fn test_archive_without_results_causes_empty_merge_side() -> Result<(), FoldError> {
    ...
    let mut results =
        DiskBackedQueue::new_from_path(non_existent_results_path.to_str().unwrap(), 10)?;
    ...
    if non_existent_results_path.exists() {
        fs::rename(&non_existent_results_path, &archive_results_path)
    ...
    let loaded_results = DiskBackedQueue::new_from_path(loaded_results_path.to_str().unwrap(), 10)?;
...
fn test_archive_should_require_results_directory() -> Result<(), FoldError> {
    ...
    assert!(
        archive_results_path.exists(),
        "Archive should have results directory"
    );

    // Load and verify by popping (len() is not reliable for reloaded queues)
    let mut loaded_results =
        DiskBackedQueue::new_from_path(archive_results_path.to_str().unwrap(), 10)?;

    let mut count = 0;
    while loaded_results.pop()?.is_some() {
        count += 1;
    }

    assert_eq!(count, 2, "Should load all results");
