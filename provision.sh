...
# Provision infrastructure for fold k8s deployment
# This script sets up the required infrastructure dependencies and secrets

NAMESPACE=${NAMESPACE:-fold}

echo "Provisioning infrastructure for fold deployment..."
...
echo "Setting up RabbitMQ message queue..."
kubectl apply -f - <<EOF
apiVersion: apps/v1
kind: Deployment
metadata:
...
echo "MinIO Console: http://localhost:9001 (kubectl port-forward service/minio 9001:9001 -n $NAMESPACE)"